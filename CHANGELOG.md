...
- M4机会扫描增量: K线摘要串行改8 worker并行+按日进程缓存(`fetch_kline_summaries`, 失败回{}同旧语义)；8×0.2s并行<1s对照测试
- M5 K线叠加持仓除权: `GET /api/quotes/{symbol}/overlays`(成本/止损/目标线+买卖点+除权标记, 空数组不抛)+`InteractiveKline`绘制(买红↑卖绿↓/除权蓝圈, markers限K线范围)+4例测试
